package io.pivotal.springtrader.portfolio.services;

import io.pivotal.springtrader.portfolio.domain.*;
...
public class PortfolioService {
	...
	@PostConstruct
	void urlCheck() {
		if (quotesUrl.indexOf("xxxx-yyyy") != -1 || accountsUrl.indexOf("xxxx-yyyy") != -1)
			throw new IllegalStateException(
					"The accounts and quotes URLs in application.yml need to be set to real CloudFoundry URLs.");
	}
	...
	public Portfolio getPortfolio(String accountId) {
		...
		createPortfolio(portfolio, orders);
	...
	private Account getAccount(String userid) {
		...
		logger.debug("Invoke " + accountsUrl + "/account?userid={userid}");
	...
	public Order addOrder(Order order) {
		...
		if (order.getOrderType() == OrderType.BUY) {
			...
			ResponseEntity<Double> result = restTemplate.getForEntity(
					accountsUrl + "/accounts/{userid}/decreaseBalance/{amount}", Double.class, order.getAccountId(),
					amount);
			return saveOrder(order, result);
		} else {
			double amount = order.getQuantity() * order.getPrice().doubleValue() - order.getOrderFee().doubleValue();
			ResponseEntity<Double> result = restTemplate.getForEntity(
					accountsUrl + "/accounts/{userid}/increaseBalance/{amount}", Double.class, order.getAccountId(),
					amount);
			return saveOrder(order, result);
		}
	...
	private Order saveOrder(Order order, ResponseEntity<Double> result) {
		if (result.getStatusCode() == HttpStatus.OK) {
			...
			throw new IllegalStateException("NOT_ENOUGH_FUND");
		}
	}
}
